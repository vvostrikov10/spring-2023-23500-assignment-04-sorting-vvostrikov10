...
void qsort2(std::vector<int> &vect, int low, int high){
    if (low >= high) return;
    int len = high - low + 1;
    int temp;
    int a = vect[low];
    int b = vect[len / 2 + low];
    int c = vect[high];
    int pivot;
    if (a >= b && b >= c || c >= b && b >= a) pivot = len / 2 + low;
    if (b >= a && a >= c || c >= a && a >= b) pivot = low;
    if (a >= c && c >= b || b >= c && c >= a) pivot = high;
    //swap pivot with last element
    temp = vect[pivot];
    vect[pivot] = vect[high];
    vect[high] = temp;
    ...
    while(i <= high){
        if (vect[i] <= vect[pivot]){
            temp = vect[right_start];
            vect[right_start] = vect[i];
            vect[i] = temp;
            right_start++;
        }
        i++;
    ...
    pivot = right_start - 1;
    qsort2(vect, low, pivot - 1);
    qsort2(vect, pivot + 1, high);
}
...
int main(int argc, char *argv[])
{
  ...
  else if (algorithm=='c'){ // the standard c++ sort
